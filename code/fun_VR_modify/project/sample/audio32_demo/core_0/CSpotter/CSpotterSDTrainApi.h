...
// Return: Success or error code
CSPDLL_API INT CSpotterSD_DifferenceCheck(HANDLE hCSpotter, INT nDiffChkLevel);				// Deprecated
CSPDLL_API INT CSpotterSD_DifferenceCheckWithRejLevel(HANDLE hCSpotter, INT nRejLevel);		// in the range of [+100, -100]
...
CSPDLL_API INT CSpotterSD_SimilarityCheck(HANDLE hCSpotter, char *lpszModelAddr, INT nSimChkLevel);				// Deprecated
CSPDLL_API INT CSpotterSD_SimilarityCheckWithRejLevel(HANDLE hCSpotter, char *lpszModelAddr, INT nRejLevel);	// in the range of [+100, -100]

// The default is 600. It is equal to the RMS amplitude and the wave amplitude is more than 4 times of that amount.
CSPDLL_API INT CSpotterSD_SetBackgroundEnergyThreshd(HANDLE hCSpotter, INT nThreshold);

// The default is 0. The range is [-100, 100].
CSPDLL_API INT CSpotterSD_SetEpdLevel(HANDLE hCSpotter, INT nEpdLevel);
