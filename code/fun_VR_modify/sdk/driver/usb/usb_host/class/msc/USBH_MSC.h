...
#define	USBH_MSC_MAX_PORT				(USBH_MAX_PORT+5)
#else
#define	USBH_MSC_MAX_PORT				(USBH_MAX_PORT+1)
#endif
...
#define	USBH_MSC_DATA_QUEUE_SIZE					10

#define	MSC_MAX_LUN												4
...
typedef	struct
{
	uint8_t				lun_status;					//!< LUN Status - 0:No Card  1:Card In
	uint32_t			lun_block_len;
	uint64_t			lun_capacity;				//!< LUN Capacity 
...
typedef	struct{
	...
	uint8_t							max_lun;					//!< Logical Unit Number of	Mass Storage Device	
	uint8_t							working_lun;
	USBH_ERR_HDL_STRUCT	*err_hdl;					//!< USB Error Handle Structure
	MSC_DEVICE_INFO			info[MSC_MAX_DEV];//!< Mass Storage Device Information	
	MSC_LUN_Struct			msc_lun_info[MSC_MAX_LUN];

#if defined (CONFIG_USBH_FREE_RTOS)	
	QueueHandle_t				QUEUE_CARD_IN;
	QueueHandle_t				QUEUE_CARD_OUT;
#endif

#if defined (CONFIG_USBH_CMSIS_OS)
	osMessageQId				QUEUE_CARD_IN;
	osMessageQId				QUEUE_CARD_OUT;
#endif
...
extern MSC_DEVICE_STRUCT MSC_DEV[USBH_MSC_MAX_PORT];

#if defined (CONFIG_USBH_FREE_RTOS)
extern QueueHandle_t	USBH_QUEUE_WAKEUP_MSC_DRV;
extern QueueHandle_t	USBH_QUEUE_MSC_TEST;
extern TaskHandle_t 	xTASK_USBH_MSC_TEST;
extern TaskHandle_t 	xTASK_HDL_MSC_DRV[USBH_MSC_MAX_PORT];

#if defined(CONFIG_CLI_APP)
extern TaskHandle_t		xTASK_CLI_MSC_TEST;
extern TaskHandle_t		xTASK_CLI_MSC_RW_TEST[USBH_MSC_MAX_PORT];	
#endif
#endif	//end of #if defined (CONFIG_USBH_FREE_RTOS)

#if defined (CONFIG_USBH_CMSIS_OS)
extern osMessageQId 	USBH_QUEUE_WAKEUP_MSC_DRV;
extern osMessageQId 	USBH_QUEUE_MSC_TEST;
extern osThreadId 		xTASK_USBH_MSC_TEST;
extern osThreadId 		xTASK_HDL_MSC_DRV[USBH_MSC_MAX_PORT];

#if defined(CONFIG_CLI_APP)
extern osThreadId			xTASK_CLI_MSC_TEST;
extern osThreadId			xTASK_CLI_MSC_RW_TEST[USBH_MSC_MAX_PORT];
#endif	
...
#if defined( CONFIG_DUAL_HOST )	
#if defined (CONFIG_USBH_FREE_RTOS)	
extern QueueHandle_t	USBH_QUEUE_WAKEUP_MSC_DRV_2;	
#endif

#if defined (CONFIG_USBH_CMSIS_OS)	
extern osMessageQId 	USBH_QUEUE_WAKEUP_MSC_DRV_2;
#endif	
...
extern uint8_t msc_random_test(uint8_t device_id, uint32_t time, uint32_t sector, uint32_t compare);
...
void msc_app_task(void const *pvParameters);
#endif
