...
#define	USBH_CONFIGURATION_NUM_MAX	0x01
#define	USBH_INTERFACE_NUM_MAX		0x03
#define	USBH_ENDPOINT_NUM_MAX		0x05
...
#if defined (CONFIG_MODULE_USB_UVC_SRAM)
#define bConfigurationDescLength	0x600
#else
#define bConfigurationDescLength	0x1000
#endif
// String definition
...
#define	PACKET_SIZE		1600	/**< Size */
#define	PACKET_MAX_CNT		10	/**< Count */
#else
...
#define MAX_USBH_RESET_COUNT		0xFFFF
#endif
...
typedef struct usbh_endpoint_descriptor_struct
{
	uint8_t		bED_Length;
...
typedef struct usbh_class_descriptor_struct
{
...
typedef struct usbh_Interface_descriptor_struct
{
...
}USBH_HID_INT_CB_Struct;

typedef struct
{
	uint32_t										class_type;
	USBH_HID_INT_CB_Struct			CB;
...
/*!
*/
typedef struct usbh_device_structure
{
	//<1>.ENUM Information
	uint32_t				device_id;
	...
	//<4>.EHCI Structure Information
	EHCI_ENDP_STRUCT			EP[MAX_QH_PER_DEVICE];				//!< EHCI ENDP Data Strucutre

	//<5>.Other	Information
	uint8_t					bData[bConfigurationDescLength];
...
typedef	enum
{
	...
	USBH_HID_CX_CMD_GETDESCRIPTOR_REPORT,
}USBH_CX_CMD_INDEX;
...
typedef	struct{
	...
	extern QueueHandle_t	USBH_QUEUE_BULK_REQ_DATA;
#endif
#if defined (CONFIG_USBH_CMSIS_OS)
#if defined( CONFIG_SNX_ISO_ACCELERATOR )
#else
	extern osMessageQId		USBH_QUEUE_STD_ISO_REQ;		
#endif
	extern osMessageQId	USBH_QUEUE_BULK_REQ[];

	extern osMessageQId	USBH_QUEUE_BULK_REQ_DATA;
#endif
...
extern uint8_t usbh_cx_clear_feature(USBH_Device_Structure *dev, USBH_CX_XFR_REQ_Struct *cx_req, BK_EP_INDEX EP);
extern uint8_t	usbh_enum(  USBH_Device_Structure * DEV,uint8_t	addr);
#if defined(CONFIG_AUTOMOUNT)
extern uint8_t usbh_force_root_plug_out(void);
#else
extern void usbh_force_root_plug_out(void);
#endif

#if defined( CONFIG_DUAL_HOST )	
extern volatile	USBH_Device_Structure USBH_DEV_2[];
extern void usbh_core_init_2(void);
extern void usbh_root_enum_2(void);
extern uint8_t	usbh_enum_2(  USBH_Device_Structure * DEV,uint8_t	addr);
extern uint8_t	usbh_parser_dev_2(USBH_Device_Structure * DEV,uint8_t FUN);
