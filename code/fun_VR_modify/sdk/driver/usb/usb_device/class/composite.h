...
struct usb_request {
	uint8_t                                 *buf;
	uint8_t									*ping_buf;	//for uac-iso-out
	uint8_t									*pong_buf;	//for uac-iso-out
	uint32_t                                length;
	uint32_t								*m_length;  //for uac-iso-out
	uint32_t								m_cnt;		//for uac-iso-out
	void                                    *parent_req;
};
...
struct usb_ep_ops {
	...
	int                                     (*dequeue) (struct usb_ep *ep, struct usb_request *req); //erk+
...
struct usb_ep {
	...
	union {
		...
		uint32_t                                packet_xfr_size; //for iso
	}s;
};
