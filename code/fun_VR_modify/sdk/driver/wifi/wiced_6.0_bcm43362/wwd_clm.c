...
resource_hnd_t wifi_firmware_clm_blob; 
...
wwd_result_t wwd_process_clm_data(void)
{
	...
	wifi_firmware_clm_blob.location = RESOURCE_IN_MEMORY;
	wifi_firmware_clm_blob.size = bcm43438A1_clm_blob_len;
	wifi_firmware_clm_blob.val.mem.data = (const char *)&bcm43438A1_clm_blob[0];
    ...
    if ((chunk_buf = (unsigned char *)malloc(size2alloc)) != NULL) {
        ...
        do {
            ...
            if ((ret = (wwd_result_t)resource_read(&wifi_firmware_clm_blob, cumulative_len, chunk_len, &size_read, chunk_buf + data_offset)) != WWD_SUCCESS) {
                break;
            }
            ...
            ret = (wwd_result_t)wwd_download2dongle(WWD_STA_INTERFACE, IOVAR_STR_CLMLOAD, dl_flag, DL_TYPE_CLM,
