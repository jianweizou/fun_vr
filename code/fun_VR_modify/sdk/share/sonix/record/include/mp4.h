...
typedef struct _CurrentMp4Info { 
	int state;
	uint32_t IndexOffset;
	
	int vdotrk_entry;
	int vdotrk_duration;
	long vdotrk_sampleCount;

	int adotrk_entry;
	int adotrk_duration;
	long adotrk_sampleCount;
}CurrentMp4Info;


typedef struct  _Mp4RecoverInfo_t{
	int    state;                 // 1:enable  0:disable
	char *save_path;      //the directory of save trailer info
	int    presion;           //unit:s
	
	unsigned int entry_count; 
	CurrentMp4Info cur_info;
	void *user_info;

	char filename_mp4info[MP4_FILENAME_LENGTH];
	char filename_vdotrak[MP4_FILENAME_LENGTH];
	char filename_adotrak[MP4_FILENAME_LENGTH];
...
void mp4_write_trailer_box(Mp4_Info_t *pMp4Info);
...
int mp4_save_trailer_init(Mp4_Info_t *pMp4Info,  Mp4RecoverInfo_t *pMp4RecoverInfo);
int mp4_save_trailer_uninit(Mp4_Info_t *pMp4Info);
int mp4_recover_trailer(Mp4_Info_t *pMp4Info, FIL *fp);
void mp4_parse_filename(const char *pathname, char *filename);
