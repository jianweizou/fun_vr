...
static aud_driver_modde_t aud_driver_mode =AUD_01_SW; //AUD_NU_SW;
...
#define SNAUD_REG_LOG(str)                          printf("%s\r\n",str);
...
static AUD_ret_t SNAUD01_sys_power_up(
                                        aud_ms_t            aud_mode,
                                        aud_mclk_path_t     mclk_path,
                                        AUD_AD_DA_t         config_ip,
                                        aud_samplerate_t    config_sr)
{
    uint16_t wdata,rdata;
    AUD01_RegTable_t reg;
   
    if( mclk_path == MCLK_PAD_TO_PLL )
    {
        /* SYS_CLK_CTRL */
        
        if( config_sr==AUD_SR48K || config_sr==AUD_SR24K || config_sr==AUD_SR12K )
        {
            wdata=0x88; /* SET FPLL output 12.288Mhz base for sys clk */
        }
        else if( config_sr==AUD_SR44p1K || config_sr==AUD_SR22p05K || config_sr==AUD_SR11p025K )
        {
            wdata=0x98; /* SET FPLL output 11.2896Mhz base for sys clk */
        }
        else if( config_sr==AUD_SR32K || config_sr==AUD_SR16K || config_sr==AUD_SR8K )
        {
            wdata=0xA8; /* SET FPLL output 8.192Mhz base for sys clk */
        }
    ...
    else if( mclk_path == MCLK_PAD_TO_SYS )
    {
        /* SYS_CLK_CTRL */
        wdata = 0;
        reg=SYS_CLK_CTRL;
    ...
    if( config_ip == SEL_ADC )
    {
        ...
        if( aud_mode == MASTER_MODE )
        {
            wdata |= SET_BIT0;
        }
        else //if( aud_mode == SLAVE_MODE )
        {
            wdata &= ~(SET_BIT0);
        }
        ...
        /* I2S_ADC_CTRL2 - master mode only */
        if( config_sr==AUD_SR48K || config_sr==AUD_SR44p1K || config_sr==AUD_SR32K )
        {            
            /* SYS_CLK=12.288Mhz,   BCLK=SYS_CLK/4(wdata)=3.072Mhz,     SR=BCLK/32=48Khz*/
            /* SYS_CLK=11.2896Mhz,  BCLK=SYS_CLK/4(wdata)=2.8224Mhz,    SR=BCLK/32=44.1Khz*/
            /* SYS_CLK=8.192Mhz,    BCLK=SYS_CLK/4(wdata)=2.048Mhz,     SR=BCLK/32=32Khz*/
            wdata=0x01;
        }
        else if( config_sr==AUD_SR24K || config_sr==AUD_SR22p05K || config_sr==AUD_SR16K )
        {
            /* SYS_CLK=12.288Mhz,   BCLK=SYS_CLK/8(wdata)=1.536Mhz,     SR=BCLK/32=24Khz*/
            /* SYS_CLK=11.2896Mhz,  BCLK=SYS_CLK/8(wdata)=1.4112Mhz,    SR=BCLK/32=22.05Khz*/
            /* SYS_CLK=8.192Mhz,    BCLK=SYS_CLK/8(wdata)=1.024Mhz,     SR=BCLK/32=16Khz*/
            wdata=0x03;
        }
        else if( config_sr==AUD_SR12K || config_sr==AUD_SR11p025K || config_sr==AUD_SR8K )
        {
            /* SYS_CLK=12.288Mhz,   BCLK=SYS_CLK/16(wdata)=0.768Mhz,     SR=BCLK/32=12Khz*/
            /* SYS_CLK=11.2896Mhz,  BCLK=SYS_CLK/16(wdata)=0.7056Mhz,    SR=BCLK/32=11.025Khz*/
            /* SYS_CLK=8.192Mhz,    BCLK=SYS_CLK/16(wdata)=0.512Mhz,     SR=BCLK/32=8Khz*/
            wdata=0x07;
        }
...
SNAUD01_MasterMode_Init( 
    ...
    bool                    amp_on)
{
    AUD_ret_t ret1=AUD_PASS, ret2=AUD_PASS, ret3=AUD_PASS;
        
    /* system power up */
    ret1 = SNAUD01_sys_power_up( MASTER_MODE, mclk_path, SEL_ADC, AUD_SR16K );  
    
    /* adc power up */
    ret2=SNAUD01_adc_power_up();
    
    /* system power up */
    ret1 = SNAUD01_sys_power_up( MASTER_MODE, mclk_path, SEL_DAC, AUD_SR16K );  
    
    /* dac power up */
    ret3 = SNAUD01_dac_power_up(dac_type,dac_depop,dac_charge_delay,amp_on);
...
AUD_ret_t SNAUD01_power_down(aud_dac_type_t dac_type)
{
    ...
    /* dac power down */
    ret2 = SNAUD01_dac_power_down(dac_type, AUD01_LDO18_BANDGAP_OFF, false, true);
...
AUD_ret_t SNAUD01_sleep(aud_dac_type_t dac_type)
{
    ...
    /* dac power down */
    ret2 = SNAUD01_dac_power_down(dac_type, AUD01_LDO18_BANDGAP_ON, true, true);
...
void AUD_Set_Driver_Mode(aud_driver_modde_t eDriverMode)
{
	aud_driver_mode = eDriverMode;
}

uint32_t AUD01_ADC_Turn_On(aud_ms_t ms_mode,aud_mclk_path_t mclk_path ,aud_samplerate_t sample_rate)
{
    AUD_ret_t ret1, ret2;

    /* sys init */
    ret1 = SNAUD01_sys_power_up( ms_mode, mclk_path, SEL_ADC, sample_rate);
    
    ret2 = SNAUD01_adc_power_up();
        
    return (AUD_ret_t)(ret1|ret2);
}

AUD_ret_t AUD01_ADC_Turn_Off(void)
{
    AUD_ret_t ret1;
    
    ret1 = SNAUD01_adc_power_down();
           
    return (AUD_ret_t)(ret1);
}

AUD_ret_t AUD01_ADC_Gain_Ctrl(ADC_Boost_t L_ADC_Boost_s, ADC_Gain_t L_ADC_Gain, ADC_Boost_t R_ADC_Boost_s, ADC_Gain_t R_ADC_Gain)
{
    uint16_t wdata,rdata;
    AUD01_RegTable_t reg;
    ...
    /* ADC_PGA_L */
    reg=ADC_PGA_L;    
    wdata=(uint16_t) (L_ADC_Boost_s<<6)+(uint16_t) L_ADC_Gain;
    AUD_RegWrite( reg, wdata);
    AUD_RegRead( reg, &rdata);
    SNAUD_DEBUG_REG("ADC_PGA_L", reg, wdata, rdata);
    if(wdata!=rdata)
    ...
    /* ADC_PGA_R  */
    reg=ADC_PGA_R;
    wdata=(uint16_t) (R_ADC_Boost_s<<6)+(uint16_t) R_ADC_Gain;
    AUD_RegWrite( reg, wdata);
    AUD_RegRead( reg, &rdata);
    SNAUD_DEBUG_REG("ADC_PGA_R", reg, wdata, rdata);
    if(wdata!=rdata)
    ...
    uint32_t            dac_delay)
{
    ...
    /* sys init */
    ret1 = SNAUD01_sys_power_up(    aud_mode,
                                    mclk_def,
                                    SEL_DAC,
                                    dac_sr);
    
    /* dac turn on */
    ret2 = SNAUD01_dac_power_up(    dac_type,
                                    dac_depop,
                                    dac_delay,
                                    false);
    ...
    bool                weakly_drv)
{
    ...
    if( weakly_drv == false )
    {
        ret1 = SNAUD01_dac_power_down(   dac_type,
                                        AUD01_LDO18_BANDGAP_OFF,
                                        false,
                                        false);
    }
...
AUD_ret_t AUD01_DAC_Volume_Control(uint8_t vol)
{
    uint16_t wdata,rdata;
    AUD01_RegTable_t reg;
    ...
    SNAUD_DEBUG_REG("DAC_GAIN", reg, wdata, rdata);
    if(wdata!=rdata)
    {
        SNAUD_REG_WRITE_FAIL("DAC_GAIN");
       return AUD_FAIL; 
    }
    SNAUD_REG_WRITE_PASS("DAC_GAIN"); 
    return AUD_PASS; 
}

AUD_ret_t AUD01_AMP_Turn_On(uint8_t vol)
{
    uint16_t wdata,rdata;
    AUD01_RegTable_t reg;
...
AUD_ret_t AUD01_AMP_Turn_Off(void)
{
    uint16_t wdata,rdata;
    AUD01_RegTable_t reg;
...
//                  AUD01 SYS API
//=============================================================================
AUD_ret_t AUD01_dpd(void)
{
    AUD_ret_t ret1=AUD_PASS;

    /* system power down */
    ret1 = SNAUD01_sys_power_down();
