...
#define	MAX_RESOLUTION_NUM		6
#define	MAX_FRAME_RATE_NUM		6
#else
#define	MAX_FORMAT_NUM				3
#define	MAX_RESOLUTION_NUM		30
#define	MAX_FRAME_RATE_NUM		7
#endif
...
#define CLEAR_ALL_STREAM_ID                     0xFF
...
}USBH_UVC_Still_Image_Frame_Descriptor_Struct;
...
typedef	struct
{
	USBH_UVC_MW_STRUCTURE		stream[USBH_UVC_DATA_QUEUE_SIZE];
	uint32_t			index;
...
typedef	struct
{
	...
	USBH_UVC_Still_Image_Frame_Descriptor_Struct	STILL_FRAME_Des;
...
{
	uint8_t                         bCurDevCnt;
	USBH_UVC_DEV_Struct             DEV[USBH_MAX_PORT*2];
...
#if defined (CONFIG_MODULE_USB_UVC_SRAM)
#define	UVC_INTERFACE_NUM_MAX	0x03
#define	UVC_ENDPOINT_NUM_MAX	0x09
#define	UVC_FRAME_NUM_MAX	0x05
#define	MAX_DES_NUM		0x06
#define	UVC_STREAMID_NUM_MAX	0x05
#else
#define	UVC_INTERFACE_NUM_MAX	0x09
#define	UVC_ENDPOINT_NUM_MAX	0x09
#define	UVC_FRAME_NUM_MAX	0x20
#define	MAX_DES_NUM		0x06
#define	UVC_STREAMID_NUM_MAX	0x20
#endif
...
typedef	struct
{
	uint8_t	bStillInftIndex;
	uint8_t	bStillFmtIndex;
	uint8_t	bStillResIndex;
}USBH_UVC_STILL_COMMIT_INFO;
...
{  
	...
	uint8_t		bMaxVideoFrameSize[4];
	uint8_t		bMaxPayloadTransferSize[4];

	uint8_t		bStillIntfNum;
	uint8_t		bStillTgtFmt;
	uint8_t		bStillTgtRes;
	uint32_t	bStillTgtWidth;
	uint32_t	bStillTgtHeight;

}USBH_UVC_STILL_CONTROL;
...
{  
	...
	USBH_UVC_STILL_CONTROL		STILL_PROBE;
	USBH_UVC_STILL_COMMIT_INFO	STILL_COMMIT;
...
extern uint32_t test_frame_count;
#endif
...
#endif
#if defined (CONFIG_USBH_CMSIS_OS)
extern void uvc_task(void const *pvParameters);
extern void uvc_app_task(void const *pvParameters);
#endif

extern void uvc_task_init(uint8_t device_id);
extern void uvc_task_uninit(uint8_t device_id);

// UVC API
extern uint32_t	uvc_start(USBH_Device_Structure	*UVC_DEV, uint8_t bFmt,	uint8_t	bRes, uint32_t dwFps,uint32_t *ptr,uint32_t size, uint8_t intf_start_index);
extern uint8_t	uvc_stop(USBH_Device_Structure *UVC_DEV, uint8_t stream_id);
extern uint8_t	uvc_still_setting(USBH_Device_Structure *UVC_DEV, uint8_t bFmt, uint32_t width, uint32_t height, uint8_t intf_start_index);
extern uint8_t	uvc_still_trigger(USBH_Device_Structure *UVC_DEV);

// UVC FUNCTION
#if defined (CONFIG_USBH_FREE_RTOS)
extern QueueHandle_t		usbh_queue_uvc_drv;
extern QueueHandle_t		usbh_queue_uvc_mw;
extern QueueHandle_t		usbh_queue_uvc_app;
extern QueueHandle_t		usbh_queue_uvc_xfr_ctrl;

//extern xTaskHandle	xTASK_HDL_UVC_DRV[USBH_MAX_PORT*2];
extern xTaskHandle	xTASK_HDL_UVC_DRV[10];
extern TaskHandle_t	xTASK_HDL_UVC_APP;
extern TaskHandle_t	xTASK_HDL_UVC_STREAM_XFR;
#endif
#if defined (CONFIG_USBH_CMSIS_OS)
extern osMessageQId			usbh_queue_uvc_drv;
extern osMessageQId			usbh_queue_uvc_mw;
extern osMessageQId			usbh_queue_uvc_app;
extern osMessageQId			usbh_queue_uvc_xfr_ctrl;

extern osThreadId				xTASK_HDL_UVC_DRV[USBH_MAX_PORT*2];
extern osThreadId				xTASK_HDL_UVC_APP;
extern osThreadId				xTASK_HDL_UVC_STREAM_XFR;
#endif

extern uint8_t get_291_h264_gop(USBH_Device_Structure *uvc_dev, uint32_t *gop);
extern uint32_t	uvc_get_info(void);
extern uint8_t uvc_set_res_val(uint16_t	wWidth,	uint16_t wHeight);
extern uint32_t uvc_get_streamid(USBH_Device_Structure *UVC_DEV);
...
extern uint8_t uvc_streamid_to_devid(uint8_t stream_id);
extern uint8_t uvc_set_streamid(USBH_Device_Structure *UVC_DEV);
extern void uvc_stream_complete(USBH_UVC_MW_STRUCTURE	*usbh_uvc_mw);
...
extern void uvc_clean_bk_buf(void);

// UVC data queue
extern USBH_UVC_DATA_QUEUE_Structure	UVC_DATA;
#if defined(	CONFIG_SNX_ISO_ACCELERATOR )
extern void uvc_process_data(USBH_Device_Structure *UVC_DEV,EHCI_ENDP_STRUCT *EP);
extern void uvc_process_err_data(USBH_Device_Structure *UVC_DEV,EHCI_ENDP_STRUCT *EP);
#else
extern void uvc_process_data(USBH_Device_Structure *UVC_DEV,EHCI_ENDP_STRUCT *EP);
#endif
