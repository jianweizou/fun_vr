...
static wiced_country_code_t          country_code  = WICED_DEFAULT_COUNTRY_CODE;
...
wiced_result_t wiced_init( void )
{
    ...
    WPRINT_WICED_INFO( ("\n\rStarting WICED v" WICED_VERSION "\r\n") );
 ...
 */
wiced_result_t wiced_join_ap( wiced_config_ap_entry_t* ap )
{
    unsigned int             a;
    int                      retries;
...
//		ap->details.SSID.length = strlen("wifi-hotspot");
//    strncpy(&ap->details.SSID.value,"wifi-hotspot",ap->details.SSID.length);
//    ap->details.security = WICED_SECURITY_OPEN;
...
wiced_result_t wiced_join_ap_specific( wiced_ap_info_t* details, uint8_t security_key_length, const char security_key[ 64 ] )
{
    ...
    if ( join_result == WICED_SUCCESS )
    {
        ...
        //wiced_sta_security_type = details->security; //enable when wwd_management_set_event_handler call.
