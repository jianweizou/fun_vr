...
wiced_result_t wiced_network_up( wiced_interface_t interface, wiced_network_config_t config, const wiced_ip_setting_t* ip_settings, wiced_config_ap_entry_t* ap )
{
    ...
    if ( wiced_network_is_up( interface ) == WICED_FALSE )
    {
        ...
        else if ( interface == WICED_AP_INTERFACE )
        {
            ...
            wiced_dct_read_unlock( soft_ap, WICED_FALSE );
						...
						wiced_ssid_t ssid =
            {
                .length =  sizeof("SNX7320_AP")-1,
                .value  = "SNX7320_AP",
            };
#endif
						result = wiced_start_ap( &ap->details.SSID, (wiced_security_t)ap->details.security, ap->security_key, 11 );
