	...
	IN	UINT			Len)
{
	...
	{
		DBGPRINT(RT_ERROR, ("[ERR]%s: TKIP MIC Error !!\n", __FUNCTION__));  /*MIC error.*/
		return (FALSE);
	}
	return (TRUE);
}
	...
	IN 		UINT16			*DataByteCnt)
{
	...
	if (pKey->KeyLen == 0)
	{
		DBGPRINT(RT_ERROR, ("%s : the key is empty)\n", __FUNCTION__));
		return FALSE;
	}
    ...
    {
		DBGPRINT(RT_ERROR, ("! WEP Data CRC Error !\n"));	 /*CRC error.*/
		return FALSE;
	}
	...
	{
		DBGPRINT(RT_ERROR, ("! TKIP MIC Error !\n"));	 /*MIC error.*/
		return FALSE;		
	}
...
/*	ARC4_CTX_STRUC ARC4_CTX;*/
	...
	/* allocate memory */
	pARC4_CTX = (ARC4_CTX_STRUC *)pvPortMalloc(sizeof(ARC4_CTX_STRUC));
	if (pARC4_CTX == NULL)
	{
