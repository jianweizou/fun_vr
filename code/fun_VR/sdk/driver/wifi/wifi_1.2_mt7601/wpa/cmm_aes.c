	...
	IN PCIPHER_KEY	pWpaKey)
{
	...
	if (pWpaKey->KeyLen == 0)
	{
		DBGPRINT(RT_TRACE, ("RTMPSoftDecryptAES failed!(the Length can not be 0)\n"));
		return FALSE;
	}
	...
	*(pData + 1) = *(pData + 1) | 0x40;
	...
	{
		DBGPRINT(RT_ERROR, ("RTMPSoftDecryptAES, MIC Error !\n"));	 /* MIC error. */
		return FALSE;
	}
	...
	INOUT UINT16 *DataLen)
{
	...
	if (pKey->KeyLen == 0)
	{
		DBGPRINT(RT_ERROR, ("%s : The key is not available !\n", __FUNCTION__));
		return FALSE;
	}
	...
	IN INT input)
{
	...
	else
	{
		...
		hex_dump((PUCHAR)"Calculate AAD", res_buf, res_len);
	}
	/* Check NONCE */
	NdisZeroMemory(res_buf, 100);
	res_len = 0;
	...
	else
	{
		...
		hex_dump((PUCHAR)"Calculate NONCE", res_buf, res_len);
	}
	/* Check CCMP-Header */
	NdisZeroMemory(res_buf, 100);
	res_len = 0;
	...
	else
	{
		...
		hex_dump((PUCHAR)"Calculate CCMP_HDR", res_buf, 8);
	}
					...
					res_buf, &res_len) == 0)
	{
		...
		else
		{
			...
			hex_dump((PUCHAR)"CCM_Encrypt", res_buf, res_len);
		}
	}
					...
					res_buf, &res_len) == 0)
	{
		...
		else
		{
			...
			hex_dump((PUCHAR)"CCM_Decrypt", res_buf, res_len);
		}
	}	
