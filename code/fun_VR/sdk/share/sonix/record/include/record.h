...
#define REC_DBG(fmt, args...) printf("\n[REC] %s : " fmt, __func__, ##args)
...
typedef void (*finishedfile_cb_t)(int type, char* finishedfile,char* renameoffile);
typedef void (*update_seed_cb_t)();

/**
* @brief structure for record parameters
*/
typedef struct  _RecRecoverInfo{
...
const char *get_lost_data_path(void);
