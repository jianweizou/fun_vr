...
{
	if (GPIO_Irq_GetPinIrq(GPIO_PORT_0_P01))
	{
		GPIO_Irq_ClearPinIrq(GPIO_PORT_0_P01);
		Motor_done_cnt = 0;
	}
}
...
unsigned char Change_Motor_PWM(void)
{
	if (Motor_Level == 0)
	{
		...
		GPIO_Irq_Init(GPIO_PORT_0);
		GPIO_Irq_SetPin(GPIO_PORT_0_P01, true, GPIO_EDGE_SENSE_RISING);
